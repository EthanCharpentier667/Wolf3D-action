...
static void move_enemy_with_collision(enemy_t *enemy)
{
    sfVector2f enemy_pos = v2f(enemy->pos.x, enemy->pos.y);
    ...
    if (is_osbtacle(new_pos_x.x, new_pos_x.y) == 0)
        enemy->pos.x = new_pos_x.x;
    if (is_osbtacle(new_pos_y.x, new_pos_y.y) == 0)
